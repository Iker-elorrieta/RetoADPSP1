...
public class EscribirXml {
	...
	public boolean generarXmlMunicipios(List<Municipios> listaMunicipios) {
	...
	public boolean generarXmlUbicaciones(ArrayList<Ubicaciones> ubicaciones) {

		File xml = new File("xmlUbicaciones.xml");
		try {
			DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
			DocumentBuilder db = dbf.newDocumentBuilder();
			Document doc = db.newDocument();
			Element eUbicaciones = doc.createElement("ubicaciones");
			doc.appendChild(eUbicaciones);

			for (Ubicaciones ubicacion : ubicaciones) {

				Element eUbicacion = doc.createElement("ubicacion");
				eUbicaciones.appendChild(eUbicacion);

				Element eCodEsp = doc.createElement("codigo_espacio");
				eCodEsp.appendChild(doc.createTextNode(String.valueOf(ubicacion.getEspacios().getCodEspacio())));
				eUbicacion.appendChild(eCodEsp);

				Element eCodMuniAuto = doc.createElement("codigo_municipio_Auto");
				eCodMuniAuto
						.appendChild(doc.createTextNode(String.valueOf(ubicacion.getMunicipios().getCodMuniAuto())));
				eUbicacion.appendChild(eCodMuniAuto);
			}
	...
	public boolean generarXmlEstaciones(ArrayList<Estaciones> estaciones) {
		...
		try {
			...
			for (Estaciones estacion : estaciones) {
				...
				Element eMunicipio = doc.createElement("municipio");
				eMunicipio.appendChild(doc.createTextNode(estacion.getMunicipios().getNombre()));
	...
	public boolean generarXmlDatos(ArrayList<Datos> datos) {
		...
		try {
			...
			for (Datos dato : datos) {
				...
				eCodEst.appendChild(doc.createTextNode(String.valueOf(dato.getEstaciones().getCodEst())));
				eDato.appendChild(eCodEst);

				Element eFecha = doc.createElement("fecha");
				eFecha.appendChild(doc.createTextNode(String.valueOf(dato.getFecha())));
				eDato.appendChild(eFecha);

				Element eHora = doc.createElement("hora");
				eHora.appendChild(doc.createTextNode(String.valueOf(dato.getHora())));
